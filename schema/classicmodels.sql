...
--
-- Host: localhost
-- Generation Time: Nov 30, 2017 at 05:42 PM
...
--
-- Stand-in structure for view `Customer Order Totals`
--
CREATE TABLE `Customer Order Totals` (
);
...
,`Contact Last` varchar(50)
,`Contact First` varchar(50)
,`Phone` varchar(50)
,`Address` varchar(50)
...
,`Zip` varchar(15)
,`Country` varchar(50)
,`Sales Rep.` varchar(101)
,`Sales Rep. Phone` varchar(61)
);
...
,`Line Description` varchar(4000)
,`Scale` varchar(10)
,`Buy Price` decimal(10,2)
,`MSRP` decimal(10,2)
,`Quantity in Stock` smallint(6)
);
...
--
-- Stand-in structure for view `Products in Order`
--
CREATE TABLE `Products in Order` (
`Order Number` int(11)
,`Date Ordered` date
,`Date Required` date
,`Date Shipped` date
...
,`In Stock` smallint(6)
,`Description` text
,`Total Cost` decimal(20,2)
);

-- --------------------------------------------------------

--
-- Stand-in structure for view `Product Statistics`
--
CREATE TABLE `Product Statistics` (
);

-- --------------------------------------------------------

--
-- Structure for view `Customer Order Totals`
--
DROP TABLE IF EXISTS `Customer Order Totals`;

CREATE ALGORITHM=UNDEFINED DEFINER=`dthomas`@`%` SQL SECURITY DEFINER VIEW `Customer Order Totals`  AS  select `customers`.`customerNumber` AS `customerNumber`,`customers`.`customerName` AS `customerName`,`customers`.`contactLastName` AS `contactLastName`,`customers`.`contactFirstName` AS `contactFirstName`,`customers`.`phone` AS `phone`,count(`orders`.`status`) AS `totalOrders`,sum((case when (`orders`.`status` = 'Shipped') then 1 else 0 end)) AS `completedOrders`,sum((case when (`orders`.`status` = 'In Process') then 1 else 0 end)) AS `inProcessOrders` from (`customers` join `orders` on((`customers`.`customerNumber` = `orders`.`customerNumber`))) group by `customers`.`customerNumber` ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`davidthomas`@`%` SQL SECURITY DEFINER VIEW `Customers`  AS  select `customers`.`customerNumber` AS `ID`,`customers`.`customerName` AS `Name`,`customers`.`creditLimit` AS `Credit Limit`,`customers`.`contactLastName` AS `Contact Last`,`customers`.`contactFirstName` AS `Contact First`,`customers`.`phone` AS `Phone`,`customers`.`addressLine1` AS `Address`,`customers`.`addressLine2` AS `Address 2`,`customers`.`city` AS `City`,`customers`.`state` AS `State`,`customers`.`postalCode` AS `Zip`,`customers`.`country` AS `Country`,concat(`employees`.`firstName`,' ',`employees`.`lastName`) AS `Sales Rep.`,concat(`offices`.`phone`,' ',`employees`.`extension`) AS `Sales Rep. Phone` from ((`customers` left join `employees` on((`customers`.`salesRepEmployeeNumber` = `employees`.`employeeNumber`))) left join `offices` on((`employees`.`officeCode` = `offices`.`officeCode`))) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`davidthomas`@`%` SQL SECURITY DEFINER VIEW `Employees`  AS  select `employees`.`employeeNumber` AS `ID`,`employees`.`lastName` AS `Last`,`employees`.`firstName` AS `First`,`employees`.`jobTitle` AS `Title`,concat(`bosses`.`firstName`,' ',`bosses`.`lastName`) AS `Supervisor`,`employees`.`email` AS `Email`,concat(`offices`.`phone`,' ',`employees`.`extension`) AS `Phone`,`offices`.`addressLine1` AS `Address`,`offices`.`addressLine2` AS `Address 2`,`offices`.`city` AS `City`,`offices`.`state` AS `State`,`offices`.`postalCode` AS `Zip`,`offices`.`country` AS `Country`,`offices`.`territory` AS `Territory` from ((`employees` left join `employees` `bosses` on((`employees`.`reportsTo` = `bosses`.`employeeNumber`))) left join `offices` on((`employees`.`officeCode` = `offices`.`officeCode`))) ;
...
CREATE ALGORITHM=UNDEFINED DEFINER=`davidthomas`@`%` SQL SECURITY DEFINER VIEW `Products`  AS  select `products`.`productCode` AS `ID`,`products`.`productName` AS `Product`,`products`.`productDescription` AS `Description`,`products`.`productVendor` AS `Vendor`,`productlines`.`productLine` AS `Product Line`,`productlines`.`textDescription` AS `Line Description`,`products`.`productScale` AS `Scale`,`products`.`buyPrice` AS `Buy Price`,`products`.`MSRP` AS `MSRP`,`products`.`quantityInStock` AS `Quantity in Stock` from (`products` left join `productlines` on((`products`.`productLine` = `productlines`.`productLine`))) ;

-- --------------------------------------------------------

--
-- Structure for view `Products in Order`
--
DROP TABLE IF EXISTS `Products in Order`;

CREATE ALGORITHM=UNDEFINED DEFINER=`dthomas`@`%` SQL SECURITY DEFINER VIEW `Products in Order`  AS  select `orders`.`orderNumber` AS `Order Number`,`orders`.`orderDate` AS `Date Ordered`,`orders`.`requiredDate` AS `Date Required`,`orders`.`shippedDate` AS `Date Shipped`,`orders`.`status` AS `Status`,`products`.`productName` AS `Name`,`details`.`quantityOrdered` AS `Quantity`,`details`.`priceEach` AS `Price Each`,`products`.`quantityInStock` AS `In Stock`,`products`.`productDescription` AS `Description`,(`details`.`quantityOrdered` * `details`.`priceEach`) AS `Total Cost` from ((`orderdetails` `details` join `orders` on((`details`.`orderNumber` = `orders`.`orderNumber`))) join `products` on((`details`.`productCode` = `products`.`productCode`))) where (`products`.`quantityInStock` > 0) order by `details`.`priceEach` desc ;

-- --------------------------------------------------------

--
-- Structure for view `Product Statistics`
--
DROP TABLE IF EXISTS `Product Statistics`;

CREATE ALGORITHM=UNDEFINED DEFINER=`dthomas`@`%` SQL SECURITY DEFINER VIEW `Product Statistics`  AS  select `products`.`productCode` AS `Product Code`,`products`.`productName` AS `Product Name`,sum(`details`.`quantityOrdered`) AS `Quantity Ordered`,count(`orders`.`orderNumber`) AS `Total Orders Containing`,(sum(`details`.`quantityOrdered`) / count(`orders`.`orderNumber`)) AS `Avg. Quantity Per Order` from ((`orderdetails` `details` join `orders` on((`details`.`orderNumber` = `orders`.`orderNumber`))) join `products` on((`details`.`productCode` = `products`.`productCode`))) group by `products`.`productCode` order by `products`.`productCode` ;
